...
CHAOS_VAR const int boneId = 31086;

static bool SafetyCheck()
{
	Ped playerPed     = PLAYER_PED_ID();
	Vector3 playerPos = GET_PED_BONE_COORDS(playerPed, boneId, 0.f, 0.f, 0.f);
...
static void SetupCamera()
{
	...
	camera = CREATE_CAM_WITH_PARAMS("DEFAULT_SCRIPTED_CAMERA", boneCoords.x, boneCoords.y, boneCoords.z, 0, 0, 0, 60,
	                                false, 2);
	ATTACH_CAM_TO_PED_BONE(camera, playerPed, boneId, 0, 0, 0, 0);
