...
// Kindly borrowed from Moxi's build
...
static void OnTick()
{
	...
	if (curTick > lastTick + 200)
	{
		...
		if (!IS_PED_IN_ANY_VEHICLE(player, false))
		{
			...
			{
				SHOOT_SINGLE_BULLET_BETWEEN_COORDS(spawnCoords.x, spawnCoords.y, spawnCoords.z + max.z - min.z,
				                                   spawnCoords.x, spawnCoords.y, spawnCoords.z, 0, true, weaponHash, 0,
				                                   false, true, 0.01);
