...
static void ApplyRandomForceToProps()
{
	for (auto const object : GetAllProps())
	{
		if (HAS_ENTITY_COLLIDED_WITH_ANYTHING(object))
		{
			...
			auto const randomXForce = static_cast<float>(GET_RANDOM_INT_IN_RANGE(-200, 200));
			auto const randomYForce = static_cast<float>(GET_RANDOM_INT_IN_RANGE(-200, 200));
			auto const randomZForce = static_cast<float>(GET_RANDOM_INT_IN_RANGE(100, 500));
