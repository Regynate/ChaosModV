...
static void SpawnTheLostAndDamned()
{
	...
	auto playerCoordinates = GET_ENTITY_COORDS(player, false);
	auto playerHeading     = GET_ENTITY_HEADING(player);
	auto playerRotation    = GET_ENTITY_ROTATION(player, 0);
	auto behindPlayer      = GetCoordinatesInFront(playerCoordinates, playerRotation, -2.0f);

	auto johnnysBike =
	    CREATE_VEHICLE(hexerModel, behindPlayer.x, behindPlayer.y, behindPlayer.z, playerHeading, false, true, false);
	auto terrysBike    = CREATE_VEHICLE(daemonModel, behindPlayer.x - 2.0f, behindPlayer.y - 2.0f, behindPlayer.z,
	                                    playerHeading, false, true, false);
	auto claysBike     = CREATE_VEHICLE(daemonModel, behindPlayer.x + 2.0f, behindPlayer.y - 2.0f, behindPlayer.z,
	                                    playerHeading, false, true, false);

	auto johnnyKlebitz = CREATE_PED_INSIDE_VEHICLE(johnnysBike, 6, johnnyKlebitzModel, -1, false, true);
	auto terryThorpe   = CREATE_PED_INSIDE_VEHICLE(terrysBike, 6, terryThorpeModel, -1, false, true);
	auto claySimmons   = CREATE_PED_INSIDE_VEHICLE(claysBike, 6, claySimmonsModel, -1, false, true);

	auto lostMcGroup   = CREATE_GROUP(0);
	...
	auto lostMcRelGroup = GET_HASH_KEY("LOST");
