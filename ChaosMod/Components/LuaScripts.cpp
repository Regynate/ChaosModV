...
#define E(x, y)                 \
	...
	const std::vector<ExposableFunc> exposables {
		...
		E("CreatePoolClonePed", CreatePoolClonePed),
		...
		E("AddEntityCleanupTracker",
		  [](Entity entity, const std::function<void()> tracker)
		  {
		...
		E("SetMinimapPosition",
		  [](const LuaVector3 &vector) { Hooks::SetMinimapPosition(Vector3(vector.X, vector.Y, vector.Z)); }),
