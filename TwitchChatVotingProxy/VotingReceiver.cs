...
namespace VotingProxy.VotingReceiver
{
    ...
    class ChatVotingReceiver : IVotingReceiver
    {
        ...
        public ChatVotingReceiver(VotingReceiverConfig config)
        {
            ...
            logger.Debug($"channel {config.UserName} oauth {config.OAuth}");

            client.Initialize(
                new Credentials(config.UserName, config.OAuth, config.ClientId, config.UserId),
                config.ChannelId
            );
        ...
        public async Task<string> StartPoll(string name, string[] options)
        {
            return await client.StartPoll(config.UserId, name, options);
        }
