        ...
        private static void Main(string[] args)
        {
            if (args.Length < 1 || args[0] != "--startProxy")
            {
                Console.WriteLine("Please don't start the voting proxy process manually as it's only supposed to be launched by the mod itself."
                    + "\nPass --startProxy as an argument if you want to start the proxy yourself for debcaugging purposes.");
            ...
            try
            {
                ...
                try
                {
                    votingReceiverConfig = new VotingReceiverConfig(config.ChannelId, config.OAuth, config.UserName, config.ChannelType, config.ClientId, config.UserId);
                }
                catch (Exception e)
                {
